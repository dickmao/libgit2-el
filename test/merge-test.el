(ert-deftest merge-analysis-ff ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (create-branch "newbranch")
      (commit-change "a" "ghijkl")
      (commit-change "a" "mnopqrs")
      (checkout "master")
      (let* ((repo (libgit2-repository-open path))
             (ref (libgit2-reference-dwim repo "newbranch"))
             (ann (libgit2-annotated-commit-from-ref repo ref)))
        (should (equal '((normal fastforward) . nil)
                       (libgit2-merge-analysis repo (list ann))))))))

(ert-deftest merge-analysis-up-to-date ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (create-branch "newbranch")
      (checkout "master")
      (let* ((repo (libgit2-repository-open path))
             (ref (libgit2-reference-dwim repo "newbranch"))
             (ann (libgit2-annotated-commit-from-ref repo ref)))
        (should (equal '((up-to-date) . nil)
                       (libgit2-merge-analysis repo (list ann))))))))

(ert-deftest merge-analysis-normal ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (create-branch "newbranch")
      (commit-change "a" "ghijkl")
      (checkout "master")
      (commit-change "a" "mnopqrs")
      (let* ((repo (libgit2-repository-open path))
             (ref (libgit2-reference-dwim repo "newbranch"))
             (ann (libgit2-annotated-commit-from-ref repo ref)))
        (should (equal '((normal) . nil)
                       (libgit2-merge-analysis repo (list ann))))))))

(ert-deftest merge-analysis-preference ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (create-branch "newbranch")
      (commit-change "a" "ghijkl")
      (checkout "master")
      (commit-change "a" "mnopqrs")
      (let* ((repo (libgit2-repository-open path))
             (config (libgit2-repository-config repo))
             (ref (libgit2-reference-dwim repo "newbranch"))
             (ann (libgit2-annotated-commit-from-ref repo ref)))
        (let ((trans (libgit2-config-lock config)))
          (libgit2-config-set-bool config "merge.ff" nil)
          (libgit2-transaction-commit trans))
        (should (equal '((normal) . no-fastforward)
                       (libgit2-merge-analysis repo (list ann))))
        (let ((trans (libgit2-config-lock config)))
          (libgit2-config-set-string config "merge.ff" "only")
          (libgit2-transaction-commit trans))
        (should (equal '((normal) . fastforward-only)
                       (libgit2-merge-analysis repo (list ann))))))))

(ert-deftest merge-base ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (setq id (rev-parse))
      (create-branch "newbranch")
      (commit-change "a" "ghijkl")
      (checkout "master")
      (commit-change "a" "mnopqrs")
      (let* ((repo (libgit2-repository-open path))
             (id1 (libgit2-reference-name-to-id repo "refs/heads/master"))
             (id2 (libgit2-reference-name-to-id repo "refs/heads/newbranch")))
        (should (string= id (libgit2-merge-base repo (list id1 id2))))))))

(ert-deftest merge-base-octopus ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (setq id (rev-parse))
      (create-branch "newbranch")
      (commit-change "a" "ghijkl")
      (checkout "master")
      (create-branch "otherbranch")
      (commit-change "a" "mnopqr")
      (checkout "master")
      (commit-change "a" "tuvwxy")
      (let* ((repo (libgit2-repository-open path))
             (id1 (libgit2-reference-name-to-id repo "refs/heads/master"))
             (id2 (libgit2-reference-name-to-id repo "refs/heads/newbranch"))
             (id3 (libgit2-reference-name-to-id repo "refs/heads/otherbranch")))
        (should (string= id (libgit2-merge-base-octopus repo (list id1 id2 id3))))
        (should (string= id (libgit2-merge-base repo (list id1 id2 id3))))))))

(ert-deftest merge-bases ()
  (let (id)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (commit-change "a" "ghijkl")
      (commit-change "a" "mnopqrs")
      (setq id (rev-parse))
      (create-branch "newbranch")
      (commit-change "a" "tuvwxy")
      (checkout "master")
      (commit-change "a" "123456")
      (let* ((repo (libgit2-repository-open path))
             (id1 (libgit2-reference-name-to-id repo "refs/heads/master"))
             (id2 (libgit2-reference-name-to-id repo "refs/heads/newbranch")))
        (should (equal (list id)
                       (libgit2-merge-bases repo (list id1 id2))))))))

(ert-deftest merge-no-conflicts ()
  (let (id-merge id-head blobid-a blobid-b blobid-b)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (setq blobid-a (rev-parse "HEAD:a"))
      (create-branch "otherbranch")
      (commit-change "b" "ghijkl")
      (setq blobid-b (rev-parse "HEAD:b"))
      (setq id-merge (rev-parse))
      (checkout "master")
      (commit-change "c" "mnopqr")
      (setq blobid-c (rev-parse "HEAD:c"))
      (setq id-head (rev-parse "HEAD"))
      (let* ((repo (libgit2-repository-open path))
             (ref (libgit2-reference-dwim repo "otherbranch"))
             (ann (libgit2-annotated-commit-from-ref repo ref)))
        (should-not (libgit2-repository-state repo))
        (libgit2-merge repo (list ann))
        (should (eq 'merge (libgit2-repository-state repo)))
        (let ((index (libgit2-repository-index repo)))
          (should (= 3 (libgit2-index-entrycount index)))
          (should (string= blobid-a (libgit2-index-entry-id (libgit2-index-get-bypath index "a"))))
          (should (string= blobid-b (libgit2-index-entry-id (libgit2-index-get-bypath index "b"))))
          (should (string= blobid-c (libgit2-index-entry-id (libgit2-index-get-bypath index "c"))))
          (should-not (libgit2-index-conflicts-p index))
          (should (string= id-merge (read-file-nnl ".git/MERGE_HEAD")))
          (should (string= id-head (read-file-nnl ".git/ORIG_HEAD")))
          (should (string= "Merge branch 'otherbranch'\n" (libgit2-repository-message repo)))
          ;; Merge not committed yet
          (should (string= id-head (rev-parse))))))))

(ert-deftest merge-conflicts ()
  (let (blobid-base blobid-theirs)
    (with-temp-dir path
      (init)
      (commit-change "a" "abcdef")
      (setq blobid-base (rev-parse "HEAD:a"))
      (create-branch "otherbranch")
      (commit-change "a" "ghijkl")
      (setq blobid-theirs (rev-parse "HEAD:a"))
      (checkout "master")
      (commit-change "a" "mnopqr")
      (setq blobid-ours (rev-parse "HEAD:a"))
      (let* ((repo (libgit2-repository-open path))
             (ref (libgit2-reference-dwim repo "otherbranch"))
             (ann (libgit2-annotated-commit-from-ref repo ref)))
        (should-not (libgit2-repository-state repo))
        (libgit2-merge repo (list ann))
        (should (eq 'merge (libgit2-repository-state repo)))
        (let ((index (libgit2-repository-index repo)))
          (should (= 3 (libgit2-index-entrycount index)))
          (should (libgit2-index-conflicts-p index))
          (should (string= blobid-base (libgit2-index-entry-id (libgit2-index-get-bypath index "a" 'base))))
          (should (string= blobid-ours (libgit2-index-entry-id (libgit2-index-get-bypath index "a" 'ours))))
          (should (string= blobid-theirs (libgit2-index-entry-id (libgit2-index-get-bypath index "a" 'theirs)))))))))
